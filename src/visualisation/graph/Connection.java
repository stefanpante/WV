...
public class Connection implements Drawable{
	...
	public void draw() {
		...
		if(firstIsOriginal){
			...
			x1 = (float) (x2 - 6 * Math.cos(corner));
			y1 = (float) (y2 - 6 * Math.sin(corner));
			
			float x3 = (float) (x1 + 3*Math.cos(corner + Math.PI/2));
			float y3 = (float) (y1 + 3*Math.sin(corner+ Math.PI/2));
			
			x2 = (float) (x1 + 3*Math.cos(corner - Math.PI/2));
			y2 = (float) (y1 + 3*Math.sin(corner -  Math.PI/2));
