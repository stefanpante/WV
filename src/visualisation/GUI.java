...
public class GUI extends PApplet {
	...
	public void setup() {
		...
		if (!Application.api) {
			...
			pub = scraper.scrapeByQuery("The ARIADNE knowledge pool system")
					.get(0);
		}
	...
	private XML readXMLOnline(String imageName) throws URISyntaxException,
	...
	private void processMenuPressed(Publication result) {
		if (result != null) {
			...
			/*
			 * if(newGraph.getNodes().size() > 0){ graph = newGraph;
			 * this.graph.setGraphLayout(new RegularForceBasedLayout(graph));
			 * this.resetTransform(); } else{ showWarning(
			 * "This Paper does not have citations in our database. \n The graph cannot be displayed."
			 * ); }
			 */
		}
		menuEnabled = false;
	...
	private void setupGUI() throws URISyntaxException, IOException,
		...
		inputField = inputController.addTextfield("search", displayWidth / 2
				- width / 2, 15, width, 30);
		CColor color = new CColor(color(0, 146, 211), color(255), color(0, 60,
				255), color(0, 146, 211), color(0));
		...
		this.play = new ShapeButton(pause, play, new PVector(
				displayWidth / 4 - 40, 15), this);
		this.search = new ShapeButton(search, search, new PVector(
				3 * displayWidth / 4 + 15, 15), this);
		this.exit = new ShapeButton(exit, exit, new PVector(displayWidth - 60,
				15), this);

		CColor col = new CColor(color(0, 146, 211), color(0, 60, 255), color(0,
				146, 211), color(0, 146, 211), color(0, 146, 211));
		slider = inputController.addSlider("zoom");
		slider.scrolled(2);
		slider.setHeight(displayHeight / 2);
		slider.setWidth(10);
		slider.setPosition(displayWidth - 50, displayHeight / 2 - displayHeight
				/ 4);
		slider.setColor(col);
	...
	public void displayWarning() {
		if (currentFrameWarning < endFrameWarning) {
			...
			rect(displayWidth / 2 - 200, displayHeight / 2 - 75, 400, 35, 5, 5,
					0, 0);
			fill(color(255));
			textSize(20);
			text("Message", displayWidth / 2 - 195, displayHeight / 2 - 75,
					400, 35);
			stroke(0, 50);
			rect(displayWidth / 2 - 200, displayHeight / 2 - 40, 400, 80, 0, 0,
					5, 5);
			textSize(15);
			fill(color(0, 146, 211));
			noStroke();

			text(warning, displayWidth / 2 - 200, displayHeight / 2 - 40, 400,
					80);
			currentFrameWarning++;
		}
	}
	...
	public void showInitialLoadingAnimation() {
		if (initialLoading) {
			...
			shape(loadingAnimation2, displayWidth / 2 - 25,
					displayHeight / 2 - 25, 100, 100);
		}
	}
